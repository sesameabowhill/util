...
get_rows($D,$h=null,$k="<p class='error'>"){global$g;$cb=(is_object($h)?$h:$g);$F=array();$E=$cb->query($D);if(is_object($E)){while($G=$E->fetch_assoc())$F[]=$G;}elseif(!$E&&!is_object($h)&&$k&&defined("PAGE_HEADER"))echo$k.error()."\n";return$F;}function
...
redirect($gd,$rd=null){if($rd!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($gd!==null?$gd:$_SERVER["REQUEST_URI"]))][]=$rd;}if($gd!==null){if($gd=="")$gd=".";header("Location: $gd");exit;}}function
query_redirect($D,$gd,$rd,$Fe=true,$Wb=true,$dc=false){global$g,$k,$b;if($Wb)$dc=!$g->query($D);$jf="";if($D)$jf=$b->messageQuery("$D;");if($dc){$k=error().$jf;return
false;}if($Fe)redirect($gd,$rd.$jf);return
true;}function
...
format_time($kf,$Jb){return" <span class='time'>(".lang(84,max(0,array_sum(explode(" ",$Jb))-array_sum(explode(" ",$kf)))).")</span>";}function
...
query_redirect($zb,$gd,$ud,true,!$_POST["dropped"]);$_b=$_!=""&&($_POST["dropped"]||queries($zb));$jb=queries($hb);if(!queries_redirect($gd,($_!=""?$sd:$td),$jb)&&$_b)redirect(null,$ud);return$_b;}function
...
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$_POST["collation"]))$pf=false;$Xc=$j;}}queries_redirect(ME."db=".urlencode($Xc),lang(161),$pf);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($_).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),lang(162));}}page_header(DB!=""?lang(48):lang(163),$k,array(),DB);$Ua=collations();$_=DB;$Ta=null;if($_POST){$_=$_POST["name"];$Ta=$_POST["collation"];}elseif(DB!="")$Ta=db_collation(DB,$Ua);elseif($u=="sql"){foreach(get_vals("SHOW GRANTS")as$uc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$uc,$z)&&$z[1]){$_=stripcslashes(idf_unescape("`$z[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
redirect($y);}}page_header($_GET["ns"]!=""?lang(49):lang(50),$k);$G=$_POST;if(!$G)$G=array("name"=>$_GET["ns"]);echo'
<form action="" method="post">
...
redirect($y);}page_header($fa!=""?lang(201).": ".h($fa):lang(202),$k);$G=$_POST;if(!$G)$G=array("name"=>$fa);echo'
<form action="" method="post">
