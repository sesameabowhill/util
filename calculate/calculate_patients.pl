...
if (@clients) {
    ...
    for my $client_identity (@clients) {
        ...
        if ($client_identity =~ m/\.xml/) {
            ...
            $client_data = XMLSource->new($client_identity);

            my $db_name = $client_data->get_db_name();
            if (DBSource->is_client_exists($db_name)) {
                printf "use database [%s]\n", $db_name;
                $client_data = XMLDBSource->new(
                    $client_data,
                    DBSource->new($db_name),
                );
            }
        } else {
            $client_data = DBSource->new($client_identity);
...
sub get_patients {
    ...
    return $self->{'dbh'}->selectall_arrayref(
        "SELECT PId, FName, LName, BDate, ".$self->{'tables'}->{'patient.active'}."<>0 AS Active FROM ".$self->{'tables'}->{'patient'}." ORDER BY 3,2",
		{ 'Slice' => {} },
    );
}
...
sub get_appointments {
    ...
    return $self->{'dbh'}->selectall_arrayref(
        "SELECT Date, MAX(ifnull(Status='confirmed',0)) AS Confirm, Duration FROM ".$self->{'tables'}->{'appointment_history'}." WHERE PId=? AND Why='moved' GROUP BY Date ORDER BY Date $order LIMIT ?",
		{ 'Slice' => {} },
        $pid,
        $number,
    );
...
sub get_patient_appointment {
    ...
    my $appointment = $self->{'dbh'}->selectrow_hashref(
        "SELECT $select_str FROM ".$self->{'tables'}{'appointment_history'}." WHERE PId=? AND Date=? AND Why='moved'",
		undef,
        $pid,
        $date,
    );
    unless (defined $appointment) {
        $appointment = $self->{'dbh'}->selectrow_hashref(
            "SELECT $select_str FROM ".$self->{'tables'}{'appointment'}." WHERE PId=? AND Date=?",
            undef,
            $pid,
            $date,
        );
