...
sub get_patient_list_by_name {
	...
	my $patients = $sth->fetchall_arrayref({});
	unless (@$patients) {
		$patients = $doctor->{db_handle}->selectall_arrayref(
			'SELECT PId FROM patients WHERE concat(FName, " ", LName) like ?',
			{ 'Slice' => {} },
...
sub get_responsible_list_by_name {
	...
	unless (@$patients) {
		$patients = $doctor->{db_handle}->selectall_arrayref(
			'SELECT RId FROM responsibles WHERE concat(FName, " ", LName) like ?',
			{ 'Slice' => {} },
			string_to_like("$fname $lname"),
		);
	}
